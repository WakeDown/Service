...
FOR
    SELECT  ( SELECT    id_contract
              FROM      dbo.srvpl_contracts c
              WHERE     c.enabled = 1
                        AND c.number = l.number
            ) AS id_contract ,
            date_start ,
            CASE WHEN l.id_device IS NULL THEN 
            ( SELECT    id_device
              FROM      dbo.srvpl_devices d
              WHERE     d.enabled = 1
                        AND d.serial_num = l.serial_num
            ) ELSE l.id_device end AS id_device ,
            
            CASE WHEN l.id_service_interval IS NULL then
            ( SELECT    id_service_interval
              FROM      dbo.srvpl_service_intervals si
              WHERE     si.enabled = 1
                        AND si.name = l.service_interval
            ) ELSE l.id_service_interval end AS id_service_interval ,
            l.id
    FROM    srvpl_devices_no_graphik_load l
    WHERE   [load] = 0
